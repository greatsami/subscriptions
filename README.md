...
2. Publish resources (migrations and config files):
    ```shell
    php artisan gratesami:publish:subscriptions
    ```

3. Execute migrations via the following command:
    ```shell
    php artisan gratesami:migrate:subscriptions
    ```
...
**Gratesami Subscriptions** has been specially made for Eloquent and simplicity has been taken very serious as in any other Laravel related aspect. To add Subscription functionality to your User model just use the `\Gratesami\Subscriptions\Traits\HasSubscriptions` trait like this:

```php
namespace App\Models;

use Gratesami\Subscriptions\Traits\HasSubscriptions;
...
```php
$plan = app('gratesami.subscriptions.plan')->create([
    'name' => 'Pro',
...
```php
$plan = app('gratesami.subscriptions.plan')->find(1);
...
$amountOfPictures = app('gratesami.subscriptions.plan_feature')->where('name', 'pictures_per_listing')->first()->value;

// Get feature value through the subscription instance
$amountOfPictures = app('gratesami.subscriptions.plan_subscription')->find(1)->getFeatureValue('pictures_per_listing');
```
...
```php
$plan = app('gratesami.subscriptions.plan')->find(2);
$subscription = app('gratesami.subscriptions.plan_subscription')->find(1);
...
```php
// Find plan feature
$feature = app('gratesami.subscriptions.plan_feature')->where('name', 'listing_duration_days')->first();
...
$subscriptions = app('gratesami.subscriptions.plan_subscription')->byPlanId($plan_id)->get();

// Get bookings of the given user
$user = \App\Models\User::find(1);
$bookingsOfUser = app('gratesami.subscriptions.plan_subscription')->ofUser($user)->get(); 

// Get subscriptions with trial ending in 3 days
$subscriptions = app('gratesami.subscriptions.plan_subscription')->findEndingTrial(3)->get();

// Get subscriptions with ended trial
$subscriptions = app('gratesami.subscriptions.plan_subscription')->findEndedTrial()->get();

// Get subscriptions with period ending in 3 days
$subscriptions = app('gratesami.subscriptions.plan_subscription')->findEndingPeriod(3)->get();

// Get subscriptions with ended period
$subscriptions = app('gratesami.subscriptions.plan_subscription')->findEndedPeriod()->get();
```

### Models

**Gratesami Subscriptions** uses 4 models:
